...
export default function TrekPage({ packages }: Props) {
  ...
  useEffect(() => {
    if (pathname && packages) {
      ...
      const foundItem = packages.find((item) => item?.fields?.title === slug);
      if (foundItem) {
        setContain(foundItem?.fields);
        console.log(foundItem?.fields);
      }
    }
  }, [pathname]);
  ...
  return (
    <section className="z-10 p-5 md:p-10 xl:p-25 bg-white">
      <motion.div
        initial={{
          opacity: 0,
          y: -60,
      ...
      <div className="flex flex-col md:flex-row md:justify-between gap-6 my-4 md:my-10">
        ...
        >
          <Image
            src={`https:${contain?.image?.fields?.file?.url}`}
            alt={contain?.title}
            fill
